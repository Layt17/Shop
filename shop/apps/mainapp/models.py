...
def get_product_url(object, viewname):#Эта функция генерирует URL для кажного продукта(т.е. object)
    # далее она вызывается в классах Smartphone и Notebook, чтобы каждому объекту класса сгенерировать свой URL
...
class Product(models.Model):
    ...
    category = models.ForeignKey(Category, verbose_name='Каткгория', on_delete=models.CASCADE)
    ...
    def save(self, *args, **kwargs):#Здесь два условия:
        #на вывод ошибки 'MinResolutionErrorException' и обрезка изображения до размеров 'MAX_RESOLUTION' если
        #изображение превышает эти размеры при добавлении.
        image = self.image
        img = Image.open(image)
        max_img_width, max_img_height = self.MAX_RESOLUTION
        min_img_width, min_img_height = self.MIN_RESOLUTION
...
class Customer(models.Model):
    ...
    addres = models.CharField(max_length=255, verbose_name='Адрес', null=True, blank=True)
...
class Notebook(Product):
    ...
    def get_absolute_url(self):
        return get_product_url(self, 'product_detail') #Эта функция прописана на 17 строке выше
...
class Smartphone(Product):
    ...
    ram = models.CharField(max_length=255, verbose_name='Оперативная память')
    sd = models.BooleanField(default=True, verbose_name='Наличие слота sd')
